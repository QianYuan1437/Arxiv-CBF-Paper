...
def paper_card(p, show_citations=False):
    ...
    badge = f'<span class="badge">🔥 {p["citations"]} citations</span>' if show_citations else f'<span class="badge date">{p.get("date","")}</span>'
    url = p.get("url", "")
    link = f'<a href="{url}" target="_blank" class="arxiv-link">arXiv →</a>' if url else ""
    abstract = p.get("abstract", "")
    abstract_html = f'<p class="abstract">{abstract}{"..." if len(abstract)==300 else ""}</p>' if abstract else ""
    return f"""
    <div class="card">
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .tab {{ padding: 0.6rem 1.5rem; border-radius: 2rem; cursor: pointer; border: 2px solid #1a1a2e; background: white; font-weight: 600; transition: all 0.2s; }}
  ...
  .card {{ background: white; border-radius: 12px; padding: 1.2rem 1.5rem; margin-bottom: 1rem; box-shadow: 0 2px 8px rgba(0,0,0,0.06); transition: box-shadow 0.2s; }}
  .card:hover {{ box-shadow: 0 4px 16px rgba(0,0,0,0.12); }}
  .card-header {{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; }}
  .badge {{ font-size: 0.8rem; padding: 0.2rem 0.7rem; border-radius: 1rem; background: #fff3e0; color: #e65100; font-weight: 600; }}
  .badge.date {{ background: #e8f5e9; color: #2e7d32; }}
  .arxiv-link {{ font-size: 0.85rem; color: #1565c0; text-decoration: none; font-weight: 600; }}
  .arxiv-link:hover {{ text-decoration: underline; }}
  h3 {{ font-size: 1rem; line-height: 1.5; margin-bottom: 0.4rem; }}
  .authors {{ font-size: 0.85rem; color: #666; margin-bottom: 0.5rem; }}
  .abstract {{ font-size: 0.85rem; color: #555; line-height: 1.6; border-top: 1px solid #f0f0f0; padding-top: 0.5rem; margin-top: 0.5rem; }}
...
<footer>Auto-updated by GitHub Actions · <a href="https://github.com/QianYuan1437/Arxiv-CBF-Paper">Source</a></footer>
<script>
  function show(id) {{
